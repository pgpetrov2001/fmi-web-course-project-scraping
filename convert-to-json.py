...
def get_tables(file, **kwargs):
    tables = camelot.read_pdf(file, pages='all', split_text=True, **kwargs)
...
def identify_and_merge_tables(tables, metadata):
    ...
    for table, meta in zip(tables, metadata):
        ...
        edge_case = any(map(lambda ref: ref == identifying_headers[i] and header == identifying_headers[i+1], optional_tables))

        if header != identifying_headers[i] and not edge_case:
            if meta['page'] != prev_page + 1:
                raise Exception(f'Encountered unexpected table {header}, when expecting {identifying_headers[i]}, and it\'s page is {meta["page"]}, while the previous page is {prev_page}')

            if i == 0:
                raise Exception(f'Encountered unexpected table {header} as the first table')
    ...
    not_covered = list(filter(lambda header: header not in result, set(identifying_headers[:-1]).difference(optional_tables)))
    if len(not_covered):
        raise Exception(f'The following tables were not found in the pdf: {not_covered}')
...
def convert_to_json(tables_from_header):
    ...
    if 'Предварителни изисквания' in tables_from_header:
        result['prerequisites'] = tables_from_header['Предварителни изисквания'].iloc[1,0]
    if 'Очаквани резултати' in tables_from_header:
        result['expected_results'] = tables_from_header['Очаквани резултати'].iloc[1,0]
    #syllabus
    result['synopsis'] = [ (description, list(map(int, horarium[0].strip().split())) if len(horarium) == 1 else list(horarium)) for description, *horarium in tables_from_header['Учебно съдържание'].iloc[2:, 1:].to_numpy() ]
...
for file in glob.glob(f'{directory}/*.pdf'):
    ...
    try:
        tables, metadata = get_tables(file, line_scale=80)
    ...
    try:
        ...
        print(f'An error occurred while extracting data from tables in file {file}: {e}')
        print('Skipping')
        continue
